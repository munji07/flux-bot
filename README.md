...
모든 명령은 `!FLUX` 접두어로 시작합니다. (`!FL`, `!flux`, `!fl` 도 가능)

### 일반 대화

```
!FLUX 오늘 저녁 메뉴 추천해줘
!FLUX 파이썬 리스트 정렬 방법 알려줘
```
...
```
!FLUX 청소 20                 # 메시지 20개 삭제
!FLUX 타임아웃 @유저 10m     # 타임아웃
!FLUX 추방 @유저              # 추방 (위험 작업 확인 필요)
!FLUX 차단 @유저              # 차단 (위험 작업 확인 필요)
!FLUX 역할부여 @유저 @역할    # 역할 부여
!FLUX 감사로그 5              # 감사 로그 조회
!FLUX 관리 도움말              # 전체 명령어 목록
```

위험한 작업(추방, 차단, 역할 변경, 보안 수준 변경 등)은 30초 내 2차 확인(`!FLUX 확인` / `!FLUX 취소`) 필요.

### 예약 메시지

```
!FLUX 10분 뒤에 회의 시작한다고 알려줘
!FLUX 예약                    # 예약 목록
```
